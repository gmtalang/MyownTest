package zhaojing.com.library.cameraview.api21;
...
public class Camera2 extends CameraViewImpl{
    ...
    private final ImageReader.OnImageAvailableListener mOnImageAvailableListener
        ...
        public void onImageAvailable(ImageReader reader) {
            Log.i("Camera2","ImageReader.OnImageAvailableListener");
    ...
    public Camera2(Callback callback, PreviewImpl preview,Context context) {
        ...
        mPreviewImpl.setCallback(new PreviewImpl.Callback() {
            @Override
    ...
    void startCaptureSession() {
        if (!isCameraOpened() || !mPreviewImpl.isReady() || mImageReader == null) {
            return;
        }
        Size previewSize = chooseOptimalSize();
        mPreviewImpl.setBufferSize(previewSize.getmWidth(), previewSize.getmHeight());
        Surface surface = mPreviewImpl.getSurface();
        try {
    ...
    private Size chooseOptimalSize() {
        ...
        for (Size size : candidates) {
            if (size.getmWidth() >= surfaceLonger && size.getmHeight() >= surfaceShorter) {
                return size;
            }
        }
    ...
    private void prepareImageReader() {
        ...
        mImageReader = ImageReader.newInstance(largest.getmWidth(), largest.getmHeight(),
    ...
    private void collectCameraInfo() {
        ...
        mPreviewSizes.clear();
        for (android.util.Size size : map.getOutputSizes(mPreviewImpl.getOutputClass())) {
